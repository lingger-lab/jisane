...
export default async function RequestDetailPage(props: PageProps) {
  ...
  const { data: request } = await adminClient
    .from('request')
    .select('id, title, detail, req_type, scope, budget_hope, category_id, created_at, client:client!inner(company, region, industry)')
    .eq('id', id)
    .eq('status', 'open')
    .single()
  ...
  const client = request.client as unknown as { company: string | null; region: string | null; industry: string | null }
