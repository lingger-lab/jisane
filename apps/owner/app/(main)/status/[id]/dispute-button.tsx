...
export function DisputeButton({ settlementId, hasOpenDispute }: DisputeButtonProps) {
  ...
  if (hasOpenDispute) {
    return (
      <div className="rounded-xl border border-warning/20 bg-warning-light p-4">
        <p className="text-sm font-medium text-warning">이의제기가 접수되어 처리 중입니다.</p>
      </div>
    )
  }
  ...
  async function handleSubmit() {
    ...
    try {
      const res = await fetch('/api/disputes', {
        ...
        body: JSON.stringify({ settlement_id: settlementId, reason: reason.trim() }),
      })
    ...
    <div className="rounded-xl border border-border-light p-4 shadow-xs">
      {!open ? (
        ...
        <div className="flex flex-col gap-3">
          <h3 className="text-sm font-semibold text-text">정산 이의 제기</h3>
          <textarea
            value={reason}
