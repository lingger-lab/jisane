...
/** 상담문의 접수(기업회원 앱) — 비로그인 허용, 로그인 시 owner 연결(강제생성 안 함). */
export async function submitConsultInquiry(
  ...
  if (!pkg || pkg.targetAudience !== 'owner' || !isConsultEligible(pkg)) {
    return { error: '유효하지 않은 서비스입니다.' }
  }

  // 로그인 상태면 owner 연결(비회원 리드도 허용 — 강제 생성하지 않음)
  ...
  if (user) {
    const { data: owner } = await adminClient
      ...
      .maybeSingle()
    ownerId = (owner?.id as string | undefined) ?? null
  }
