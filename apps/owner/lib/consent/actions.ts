...
/** 마이페이지 마케팅 수신 토글 — 기업회원. 연락처(휴대폰)를 키로 consent_log에 기록. */
export async function setMarketingConsent(next: boolean): Promise<{ ok?: boolean; error?: string }> {
  ...
  const { data: owner } = await adminClient
    ...
    .maybeSingle()
  if (!owner) return { error: '회원 정보를 찾을 수 없습니다.' }

  const phone = normalizePhone(owner.contact as string | null)
  if (!phone) return { error: '휴대폰 번호를 먼저 등록해주세요.' }

  const args = { phone, ownerId: owner.id as string, source: 'mypage' as const }
