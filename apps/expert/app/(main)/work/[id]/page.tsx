...
import { ReviewDisputeButton } from './dispute-button'
...
export default async function WorkDetailPage(props: PageProps) {
  ...
  // 리뷰 + 이의제기 조회 (done 상태용)
  let reviewForDispute: { id: string } | null = null
  let hasOpenReviewDispute = false
  if (d.status === 'done') {
    const { data: review } = await adminClient
      .from('review')
      .select('id')
      .eq('deal_id', dealId)
      .in('author_type', ['owner', 'admin'])
      .limit(1)
      .single()
    if (review) {
      reviewForDispute = review
      const { data: disputes } = await adminClient
        ...
        .eq('status', 'open')
        .limit(1)
      hasOpenReviewDispute = (disputes && disputes.length > 0) || false
    }
  }
    ...
    <div className="flex flex-1 flex-col px-4 py-5 sm:px-6 sm:py-8 animate-fade-in">
      ...
      {d.status === 'done' && (
        <div className="mb-4 flex flex-col gap-3">
          ...
          {reviewForDispute && (
            <ReviewDisputeButton reviewId={reviewForDispute.id} hasOpenDispute={hasOpenReviewDispute} />
          )}
        </div>
      )}
