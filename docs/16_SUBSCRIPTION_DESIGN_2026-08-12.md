...
>
> **구현 모델 정책(하이브리드)**: 금전·동시성·보안·마이그레이션 크리티컬은 주력(Opus 4.8)+적대적 검증, UI/카피/스캐폴딩은 fable5(§9-A).
...
## 9-A. 구현 모델 정책 (하이브리드 — 2026-08-12 확정)
- **주력 모델(Opus 4.8) + 적대적 검증**: 금전·동시성·보안·마이그레이션 크리티컬 — pricing 단일소스(§11.1), 크레딧 원장·RPC(§5·§11.5), 빌링 멱등·orderId(§3·§11.4), 정산 원자성(P1-17)·환불↔적립 clawback(§11.6), RLS·billing_key(§2.4·§11.7), 모든 0030+ 마이그레이션. → §11 실패모드가 치명이라 최강 추론력·red-green·의사환경 게이트를 여기 집중.
- **fable5**: UI 프리미티브(shadcn 도입 §12.2)·화면 리디자인(§12.4)·공개 카피·스캐폴딩·테스트 보일러플레이트 등 저위험·기계적 부분. 산출물은 주력 모델이 리뷰.
