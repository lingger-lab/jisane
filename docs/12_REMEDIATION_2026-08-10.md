...
라운드4 — G 편집분 (파일 무겹침 병렬 2에이전트, 통합 게이트 vitest 140/140 · tsc 0×4 · eslint 0):

| 배치 | 감사 항목 | 조치 | 커밋 |
|---|---|---|---|
| G 시각·카피 | UX P3-2/8/14/15/54/57/58/61/62/68/78 | 반응형 그리드 4곳, 하드코딩 색→토큰 4곳(금배지·별점은 리포 관례 기준 교체 — 가시적 색 변화 명시), 오타·₩0→무료·무라벨 +N 표기 | `d8dfced` |
| G 상태·피드백 | UX P2-50, P3-5/25/42/43/45/48/49/73 | 결제 토스트 수동닫기 유지/8s(toast-policy 순수함수), 스켈레톤 3페이지 신설+admin TAB_GROUPS 공유 소스(드리프트 원천 차단), 인라인 검증(onSubmit 선검증·role=alert), 칩 상한·글자수 캡 피드백(char-limit-notice 공용) | `e574783` |

**G 잔여 = 의사결정 4건만** (요율 용어 P3-17·요율표 공개 P2-11·개인정보 책임자 P2-12·전자상거래 고지 P3-22).
...
## 5. 검증 현황 (정직 기록 — 2026-08-12 갱신)

- **로컬 Supabase 스택: 기동 가능해짐** (구 기록 "config.toml 드리프트로 기동 불가"는 현 CLI
  2.113.0에서 재현 안 됨). `npx supabase start`로 마이그레이션 전 체인(0001~0029) clean apply 확인.
  단, fresh 스택은 **0029 GRANT 마이그레이션이 필수** — 새 Supabase 기본권한 변경으로 API 롤
  DML이 자동 부여되지 않음(`f014408` 참조). 프로드는 구 기본권한 시절 생성이라 무관.
- **브라우저 런타임 스모크 실시** (2026-08-12, 로컬 스택 + 시드): 3앱 dev 기동, owner 랜딩/
  experts/services · expert 랜딩/requests · admin 랜딩 — 콘솔 에러 0, 시드 데이터 정상 렌더.
  이 과정에서 확인된 것: **ErrorState가 실제 에러 시 fake-empty 대신 표시됨**(자연 에러 주입),
  **P2-61 실패 캐시 재현→수정**(landing-stats 실값 렌더 확인). 앱 env는 `.env.example` 기준
  `.env.local`(gitignored)로 구성.
- **여전히 미실시**: 인증 플로우(OAuth는 로컬 provider 설정 필요)·결제 플로우 E2E, 동시성 CAS의
  실부하 병렬 레이스, 스크린리더 실낭독. 스테이징 스모크 대상.
- **마이그레이션의 프로드 적용**: 로컬 fresh에서만 검증. 0019는 파괴적 리셋을 포함하므로 프로드
  직접 적용 금지 — 스테이징 선행 필수(라이브는 이미 수동 반영됨). 0029는 멱등 GRANT라 재적용 무해.
